...
/**    private calendar: NgbCalendar,
  ...
  ngOnInit() {
    ...
    /* this.startEndDates = this.fb.group({
...
*/
 ...
 //  this.startEndDatesForm.valueChanges.subscribe(response => console.log(this.selected));
  ...
  dateRange() {
    localStorage.setItem('object', JSON.stringify(this.startEndDates));
  ...
  generateDaily() {
    const dailyDate = [];
    const publicDate =[""];
console.log(this.startEndDatesForm.value.startDate._i);
console.log(moment(this.currentDate));
console.log(dailyDate.push( moment(this.currentDate).format('dddd, MMMM Do YYYY')));
    this.currentDate = moment(this.startEndDatesForm.value.startDate);
    this.occurences = this.startEndDatesForm.value.occurences;
    if(this.startEndDatesForm.value.occurences) {
     while (moment( dailyDate.length) <= moment(this.startEndDatesForm.value.occurences)) {
        if (moment(this.currentDate).isoWeekday() !== 6 && moment(this.currentDate).isoWeekday() !== 7) {
          dailyDate.push( moment(this.currentDate).format('dddd, MMMM Do YYYY'));
 ...
 /*
  ...
  generateDaily() {
    ...
    generateDaily1() {
      ...
      this.occurences = this.startEndDatesForm.value.occurences;
      if(this.startEndDatesForm.value.occurences) {
       while ( dailyDate.length <= moment(this.startEndDatesForm.value.occurences)) {
          if (moment(this.currentDate).isoWeekday() !== 6 && moment(this.currentDate).isoWeekday() !== 7) {
